...
from server.globals.constants import TextPosition, DIMENSIONS
...
class Type(models.Model):
    ...
    default_primary_dimensions = models.CharField(choices=DIMENSIONS_CHOICES, default=DIMENSIONS.WIDTH, max_length=255)
...
class Element(models.Model):
    ...
    local_link = models.CharField(max_length=255, blank=False, null=False)
    ...
    rotate_degrees = models.IntegerField(blank=True, default=0)
    max_stitch_width = models.FloatField(blank=True, null=True)
    min_stitch_width = models.FloatField(blank=True, null=True)
    text_position = models.PositiveIntegerField(choices=TEXT_POSITION_CHOICES, default=TextPosition.CENTER)
    ...
    def get_image_url(self):
        ...
        return f"{base_url}/{self.local_link}.png"

    def __str__(self):
        return f"{self.type} - {self.local_link} | ID: {self.id}"
...
class TrainingData(models.Model):
    ...
    def __str__(self):
        return f"{int(self.container_width)}/{int(self.container_height)} | {self.element} | [{self.position_x}, {self.position_y}, {self.width}, {self.height}]  | Product: {self.product.id}"
